...
// 13 Nov  2013    Peter Babalis  Trigger Chain
 ...
 trigger Lead_aInsertaUpdate on Lead (before insert , before update, after insert, after update) {
    // Call AWE  after Insert
    if(trigger.isAfter && trigger.isInsert){
        LeadAWEtriggerHandler.afterInsert(Trigger.new);
    }
   // Call AWE  after Update
    if(trigger.isAfter && trigger.isUpdate){
        LeadAWEtriggerHandler.afterUpdate(Trigger.new,Trigger.oldMap);
    }
    if(Trigger.isInsert && Trigger.isBefore){
    	for(Lead ld : Trigger.new){
    		ld.Territory__c = 'a0w70000007jZSE'; //Japan Named - NSG - Koji Yamaguchi
    	}
    }
    if(Trigger.isUpdate && Trigger.isBefore){
    	for(Lead ld : Trigger.new){
    		ld.Territory__c = 'a0w70000007MvKI'; //Japan Named - NSG - Koji Yamaguchi
    	}
    }
}
/*
